import pygame, spritesheet, random; from math import sqrt
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, deltaTime, eventQueue):
        ...
        if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and self.rect.y < self.dimensions[1] - self.size[1]:
            moveY -= self.speed * deltaTime * self.mult
        if (keys[pygame.K_a] or keys[pygame.K_LEFT]) and self.rect.x > 0:
            moveX -= self.speed * deltaTime * self.mult
        if (keys[pygame.K_d] or keys[pygame.K_RIGHT]) and self.rect.x < self.dimensions[0] - self.size[0]:
            moveX += self.speed * deltaTime * self.mult

        if moveX != 0 and moveY != 0: # Makes the player displacement consistent if moving in both x and y directions
            moveX *= sqrt(2)/2
            moveY *= sqrt(2)/2

        if (keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT] or keys[pygame.K_SPACE]) and self.boost > 0: # Increases distance moved by player if speed boost is active
            moveX *= 1.8
            moveY *= 1.8
        ...
        for i in eventQueue: # Plays/stops boost sound when the player is boosting
            if i.type == pygame.KEYDOWN and (i.key == pygame.K_LSHIFT or i.key == pygame.K_SPACE or i.key == pygame.K_RSHIFT):
                boostSound.play(-1)
            elif i.type == pygame.KEYUP and (i.key == pygame.K_LSHIFT or i.key == pygame.K_SPACE or i.key == pygame.K_RSHIFT) or self.boost <= 0:
                boostSound.stop()
